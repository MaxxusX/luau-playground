...
<div class="flex flex-col h-full bg-(--bg-primary) overflow-hidden">
	...
	<main class="flex-1 flex flex-col min-h-0 overflow-hidden">
		...
		<div
			class="flex-1 min-h-0 overflow-hidden flex {$showBytecode ? 'flex-col md:flex-row' : (
				''
			)}"
		>
			<!-- Editor - takes remaining space -->
			<div
				class="min-w-0 min-h-0 overflow-hidden {$showBytecode ?
			...
			{#if $showBytecode}
				<div
					class="h-1/2 md:h-full md:w-[min(50%,64ch)] md:shrink-0 min-w-0 min-h-0 overflow-hidden"
				>
					<BytecodeView />
				</div>
			{/if}

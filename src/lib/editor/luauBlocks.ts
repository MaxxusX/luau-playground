...
function startsWithStructuralCloser(rawLine: string): boolean {
	...
	return (
		...
		first === "]" ||
		first === "}" ||
		/^end\b/.test(normalized) ||
		/^until\b/.test(normalized)
	);
}
...
export const luauEnterKeymap = [
	{ key: "Enter", run: insertLuauSmartEnter, shift: insertNewlineAndIndent },
];

<script lang="ts">
	...
	let visibleStart = $derived(
		Math.max(
			0,
			Math.floor(Math.max(0, scrollTop - VERTICAL_PADDING_TOP) / ROW_HEIGHT) - OVERSCAN_ROWS
		)
	);
	let visibleEnd = $derived(
		Math.min(
			parsedLines.length,
			Math.ceil(Math.max(0, scrollTop - VERTICAL_PADDING_TOP + viewportHeight) / ROW_HEIGHT) +
				OVERSCAN_ROWS
		)
	);
	...
	function highlightAsmArgs(args: string): string {
		return (
			args
				...
				.replace(
					/\b(r[0-9]{1,2}[dwb]?|[re]?[abcd]x|[re]?[sd]i|[re]?[sb]p|[re]?ip|xmm\d+|ymm\d+|zmm\d+)\b/gi,
					'<span class="hl-constant">$1</span>'
				)
				// ARM registers: x0-x30, w0-w30, q0-q31, v0-v31, sp, lr, etc.
				.replace(
					/\b([xwqvsd]\d{1,2}|sp|lr|pc|fp)\b/gi,
					'<span class="hl-constant">$1</span>'
				)
				...
				.replace(/\[([^\]]+)\]/g, (match, inner) => {
					...
					const highlighted = inner
						.replace(
							/\b(r[0-9]{1,2}[dwb]?|[re]?[abcd]x|[re]?[sd]i|[re]?[sb]p|[re]?ip|[xwqvsd]\d{1,2}|sp|lr|pc|fp)\b/gi,
							'<span class="hl-constant">$1</span>'
						)
						.replace(
							/\b(0x[0-9a-fA-F]+|\d+h?)\b/g,
							'<span class="hl-constant">$1</span>'
						)
	...
	function highlightLine(line: string, type: string): string {
		...
		if (type === "ir-comment") {
			...
			if (!escaped.includes("hl-ir-op")) {
				escaped = escaped.replace(/^(#\s+)(\w+)(\s+.*)$/, (_, prefix, op, args) => {
					const highlightedArgs = highlightIrArgs(args);
	...
	async function refreshBytecode(
		...
		try {
			const result = await getBytecode(
				code,
				optimizationLevel,
				bytecodeDebugLevel,
				format,
	...
	function scrollToCursorLine(index: number) {
		...
		if (targetScrollTop != null) {
			...
			const behavior =
				distance <= viewport * AUTO_SCROLL_MAX_DISTANCE_VIEWPORTS ? "smooth" : "auto";
			bytecodeContainer.scrollTo({ top: targetScrollTop, behavior });
...
{#if $showBytecode}
	<div
		class="flex flex-col h-full border-t sm:border-t-0 sm:border-l border-(--border-color) bg-(--bg-editor)"
	>
		<!-- Header -->
		<div
			class="flex items-center justify-between px-3 py-2 border-b border-(--border-color) bg-(--bg-secondary) shrink-0"
		>
			<div class="flex items-center gap-2">
				<span class="text-sm font-medium text-(--text-primary)"
					>{getFormatLabel(outputFormat)}</span
				>
				{#if isLoading}
			...
			<Button
				...
				class="h-6 w-6 p-0 min-w-0"
				title="Close bytecode view"
			>

...
{#if shouldShow}
	...
	>
		...
		<!-- Header -->
		<div
			class="flex items-center justify-between px-2 sm:px-3 py-1.5 border-b border-(--border-color) bg-(--bg-tertiary) shrink-0"
		>
			<button
		...
		{#if isExpanded}
			<div
				class="flex-1 overflow-auto p-1 py-0 sm:p-2 sm:py-1 lg:p-3 lg:py-2 font-mono text-xs sm:text-sm min-h-0"
			>
				{#if $output.length === 0}

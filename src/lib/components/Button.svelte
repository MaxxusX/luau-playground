<script lang="ts">
	...
	const baseClasses =
		"inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--accent)] disabled:pointer-events-none disabled:opacity-50 [&>svg]:shrink-0";
...
<button class={classes} {disabled} {type} {title} {onclick} {popovertarget} {...restProps}>
	{#if children}
		{@render children()}
	{/if}
</button>

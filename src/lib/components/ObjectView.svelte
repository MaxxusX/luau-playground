<script lang="ts">
	...
	const entries = $derived.by(() => {
		...
		if (value.isArray) {
			return (value.value as LuauValue[]).map((v, i) => ({ key: String(i + 1), value: v }));
		}
		return Object.entries(value.value as Record<string, LuauValue>).map(([k, v]) => ({
			key: k,
			value: v,
		}));
	});
...
--><span
	...
	>{#if isTable}{#if keyName}<span class="ov-key">{formatKey(keyName)}</span
			...
			>{" = "}{/if}<span class={getPrimitiveClass()}>{getPrimitiveText()}</span>{/if}</span
>
